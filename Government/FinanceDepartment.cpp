...
FinanceDepartment::FinanceDepartment(std::vector<CitizenInterface *> residentsList, std::vector<CommercialBuilding *> commercialBuildingsList)
{
    // Needs this one done
}
...
double FinanceDepartment::delegateRequestForCollectionOffPropertyTax()
{
    ...
    for (Citizen *resident : residentsList)
    {
        if (resident)
        {
            ...
            if (resident->getPropertyOwnershipStatus() == true)
            {
