...
int CitizenController::getEmployedCitizensAmount()
{
    ...
    for ( int i = 0 ; i < this->amountOfCitizens ; i ++)
    {
        if ( this->ListOfCitizens[i]->isCitizenEmployed() == true)
        {
            Res++ ; 
        }
    }
...
int CitizenController::getamountPropertyOwnersAmount()
{
    ...
    for ( int i = 0 ; i < this->amountOfCitizens ; i ++)
    {
        if ( this->ListOfCitizens[i]->isCitizenPropertyOwner() == true)
        {
            Res++ ; 
        }
    }
...
float CitizenController::GetTaxableIncome()
{
    ...
    for ( int i = 0 ; i < this->amountOfCitizens ; i ++)
    {
        this->TaxableIncome += ( this->ListOfCitizens[i]->getSalary() * this->ListOfCitizens[i]->getCurrTaxRate() ) ; // Is the Tax rate format 0.4 or 40 ? For now , taking it as 0.4
    }
